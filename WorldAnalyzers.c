...
// Gets the distance from bot to a location
int getDistanceToLocation(botPtr b, locPtr l) {
	...
	printf("getDIstanceToLocation dieed up exiting\n");
	exit(1);
}
...
int getDistanceToBuyer(locPtr l, locPtr* buyer, int index)
{
	...
	return 0;
	printf("getDistanceToBuyer dieed up exiting\n");
	exit(1);
}
	...
	int* distToBuyer, locPtr* desiredBuyer)
{
	...
	int distBotToSeller = abs(getDistanceBetweenLocs(b->location, seller));
	...
	while((distance = getDistanceToBuyer(seller, &buyer, buyerCount)) != 0)
	{
		...
		int wantToSell = 0;
		int diver = getBuyerDivider(b, buyer, &wantToSell);
		int newBuyerQuantity = mmax(buyer->quantity / diver, buyer->quantity - wantToSell);
		...
		score -= getPenaltyForFarnessDistClosestFuel(b, distance, seller);
		/*
		if (score < 0)
		{
		...
		*/
		...
		if (b->location != seller)
			numOfTurnsRequired += numOfTurnsForDist(b, abs(distBotToSeller)) + 2;
		// If theres not enough turns for buyer
		if (numOfTurnsRequired > b->turns_left)
			score = 0;
		// if theres not enough fuel 
		if (b->fuel < abs(distance) && b->turns_left < 4)
			score = 0;

		// if theres not enough fuel including refuel
		if (b->fuel < abs(distance) + abs(distBotToSeller) && b->turns_left < 6)
			score = 0;
		// If the buyer is impossibly out of reach, zero it
		if (fuelLeftInWorld + b->fuel < abs(distance))
			score = 0;

		printf("%s -> %s Profit: %d Q: %d Numturns: %d Penalty: %d\n",
			seller->name, buyer->name, score, guessedQuantity, numOfTurnsRequired, getPenaltyForFarnessDist(b, distance));
...
int getSellerRangePenalty(botPtr b, locPtr seller, int distance, int score, int nOfLocations)
{
	...
	// If it's already stranded, #YOLO
	if (b->location == seller 
		&& abs(distToNearestFuelLoc) + distance > b->fuel)
	{
		printf("ON a seller already stranded by nearest fuel loc, so YOLO\n");
		return 0;
	}
	...
	if (b->turns_left > 10 && getFuelLeftInWorld(b) > b->fuel_tank_capacity)
	{
		...
		// Already stranded? yolo time weeeeeeeeeeeeee
		if (decentServo && distToDecentFuel + distance > b->fuel && b->location == seller)
			return 0;


		// same for if we can't make a journey to the seller and fuel station with
		// current fuel
		if (decentServo && distToDecentFuel + distance > b->fuel)
			return score;

		// If there is a decent servo and we need it, but it doesn't have enough fuel
		// can't go here
		if (decentServo 
			&& distToDecentFuel + distance > b->fuel
...
int getBuyerProfit(botPtr b, locPtr buyer, int* quantityToSell)
{
	...
	for (struct cargo* cargoIter = b->cargo; cargoIter != NULL; cargoIter = cargoIter->next)
	{
		if (!strcmp(cargoIter->commodity->name, buyer->commodity->name))
		{
			...
			int wantToSell = 0;
			buyerDivider += getBuyerDivider(b, buyer, &wantToSell);
			
			int buyerQuantity = buyer->quantity;
			int guessQuantity = mmin(cargoIter->quantity, mmax(buyerQuantity - wantToSell,  buyerQuantity / buyerDivider));
			...
			*quantityToSell = mmin(cargoIter->quantity, buyerQuantity);
			...
			// Multiply the profit of a buyer by the turns left, since it opens up cargo space
			int profitTurnMultiplier = b->turns_left / 3 - 2;
			if (profitTurnMultiplier < 1) profitTurnMultiplier = 1;

			return profit * profitTurnMultiplier;
...
// Gets the penalty of a buyer for pushing the limits of the fuel/turns
int getBuyerRangePenalty(botPtr b, locPtr buyer, int distance, int score)
{
	...
	// If bot is already at a buyer, there is no range penalty
	if (b->location == buyer)
	{
		return 0;
	}
							...
							int* fuelToBuy)
{
	...
	if ((b->fuel > maxFuelRequired || b->fuel > b->maximum_move * 2 / 5 * b->turns_left)
		&& b->fuel > desiredDistance )
		return 0;
	...
	// Guess the quantity of fuel that will be remaining by the time we get there
	guessQ = mmax(servo->quantity / servoBotDivisor, servo->quantity - wantedFuelByBots);
	...
	if (b->location == servo && b->turns_left > 15 && b->fuel < b->fuel_tank_capacity / 10
		&& guessQ > b->fuel_tank_capacity - b->fuel)
	{
		*fuelToBuy = mmax(b->maximum_move * 2 / 5 * b->turns_left, b->fuel_tank_capacity / 10);
	...
	// Gets the distance to the nearest servo from the desired destination
	...
	// If the fuel at fuel station plus bot's fuel is less than half the tank capacity
	// and the fuel at fuel station plus bot's fuel is less than the distance from the destination to the nearest fuel statoin times 2
	// Then assume fuel station is useless
	...
	if (scores[desiredIndex][3] > 0 && b->turns_left > minimumUsefulMoves)
	{
		...
		if (stationWorthMultiplier < 1)
			stationWorthMultiplier = 1;
		score += scores[desiredIndex][3] 
			* stationWorthMultiplier 
...
// Calculates the worht of a dump based on commodities held that
// Seolhyun doesn't feel like selling

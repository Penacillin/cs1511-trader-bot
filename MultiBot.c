...
// Gets the number of bots at a location
...
// Gets the number of bots with empty cargo at a location
...
// Gets number of bots with empty cargo in a specific region
...
// Gets the number of bots at a location with a specific commodity
...
/// Gets the total number of bots in the world
...
// Gets the number of bots withing a specific region of a location
...
// Gets the number of bots withing a region who are below a certain level of
// fuel
...
// Gets the number of bots within a specific region of a location who 
// have a certain commodity
int getNumOfBotsWithinRegionWithCom(locPtr l, int region, struct commodity* com, int* totalQ)
{
	int counter = 0;
	int curr = 0;
	...
	for (;
		...
		iterForward = iterForward->next, iterBackward = iterBackward->previous)
	{
		counter += numBotsAtLocWithCom(iterForward, com, totalQ) 
			+ numBotsAtLocWithCom(iterBackward, com, totalQ);
...
{
	int botsOnBuyers = numBotsAtLocWithCom(buyer, buyer->commodity, totalBuyerQ);
	if (b->location == buyer && botsOnBuyers > 0)
		return botsOnBuyers;
	
	int turnsRequired = numOfTurnsForDist(b, getDistanceBetweenLocs(b->location, buyer));
	int totalQ = 0;
	int botsEqualToUs = getNumOfBotsWithinRegionWithCom(buyer, turnsRequired * b->maximum_move, buyer->commodity, totalBuyerQ);
